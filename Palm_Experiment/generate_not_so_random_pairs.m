...
[PV, VV] = meshgrid(positions, velocities);
PVVV = cat(2, PV', VV');

pairs = reshape(PVVV, [], 2);
...
if ~group
    ...
    randomizedMultiplePairs = [];

    for i = 1:trialCount2
        % Group as 18 (6 vel 3 pos)
        pairsToConcat = pairs(randperm(size(pairs, 1)), :);
        ctr = 1;
        while i ~= 1 && (pairsToConcat(1, 1) == randomizedMultiplePairs(end, 1) || pairsToConcat(1, 2) == randomizedMultiplePairs(end, 2))
            pairsToConcat = pairs(randperm(size(pairs, 1)), :);
            ctr = ctr + 1
        end

        randomizedMultiplePairs = [randomizedMultiplePairs; pairsToConcat];
    end
